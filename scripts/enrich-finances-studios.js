...
async function main() {
  const companies = await prisma.studio.findMany({
    where: {
      inn: { not: null },
    },
    orderBy: { name: "asc" },
  ...
  // Фильтруем тех у кого нет выручки в companyData
  const toProcess = companies.filter(s => !s.companyData?.revenue);

  const totalAll = await prisma.studio.count({ where: { inn: { not: null } } });
  const alreadyDone = totalAll - toProcess.length;

  console.log(`\n💰 Checkko: финансы СТУДИЙ`);
  console.log(`   Всего с ИНН: ${totalAll}`);
  console.log(`   Уже с финансами: ${alreadyDone}`);
  console.log(`   Осталось: ${toProcess.length}\n`);

  if (toProcess.length === 0) {
    console.log("✅ У всех студий уже есть финансы!");
